...
# Run one cargo command under a GLOBAL lock so two cargo (or trunk)
...
# deadlocking.
#
#   scripts/serial.sh cargo test -p suduko-grid
...
# PID so a lock orphaned by a SIGKILL'd build (its EXIT trap never ran)
