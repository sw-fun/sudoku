...
pub use suduko_techniques::Candidates;
...
pub fn candidates(shown: &[u8; CELL_COUNT]) -> Candidates {
    candidates_with(shown, &[])
}

/// Candidate state for shown values minus an elimination layer
/// (accumulated solver removals, not yet placements).
#[must_use]
pub fn candidates_with(shown: &[u8; CELL_COUNT], eliminated: &[(usize, u8)]) -> Candidates {
...
pub fn find_all(shown: &[u8; CELL_COUNT]) -> Vec<Annotation> {
    find_all_in(&candidates(shown))
}

/// `find_all` over prebuilt candidates (e.g. minus an elimination
/// layer).
#[must_use]
pub fn find_all_in(cands: &Candidates) -> Vec<Annotation> {
    let mut all = Vec::new();
    all.extend(singles::naked_singles(cands));
    all.extend(singles::hidden_singles(cands));
    all.extend(locked::pointing_all(cands));
    all.extend(locked::claiming_all(cands));
    all.extend(pairs::naked_pairs(cands));
    all.extend(pairs::hidden_pairs(cands));
    all.extend(x_wing::x_wings(cands));
    all.extend(xy_wing::xy_wings(cands));
